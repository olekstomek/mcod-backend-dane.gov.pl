...
#     def test_noargs_get(self, client, applications):
...
#
#     def test_query_get(self, client, applications):
...
#     def test_valid_get(self, client, application):
#         resp = client.simulate_get(f'/applications/{application.id}')
#         assert HTTP_OK == resp.status
#
#         for key in ('data', 'links', 'meta'):
...
#
#     def test_get_invalid_id(self, client, application):
#         resp = client.simulate_get('/applications/!nV')
#         assert HTTP_NOT_FOUND == resp.status
#
#         resp = client.simulate_get(f'/applications/{application.id + 10}')
#         assert HTTP_NOT_FOUND == resp.status
#
#     def test_get_deleted(self, client, application):
#         del_id = application.id
#         application.delete()
#
#         resp = client.simulate_get(f'/applications/{del_id}')
#         assert HTTP_NOT_FOUND == resp.status
#
#     def test_thumbnail(self, client, application):
#         resp = client.simulate_get(f'/applications/{application.id}')
#         assert HTTP_OK == resp.status
#
#         for key in ('data', 'links', 'meta'):
...
#     def test_valid_get(self, client, application):
#         resp = client.simulate_get(f'/applications/{application.id}/datasets')
#         assert resp.status == HTTP_OK
#
#         assert len(resp.json['data']) == 0
#         # TODO dorobić fixture dataset dla application i sprawdzic jego pola
#
#         assert all((key in resp.json['links']) for key in ('first', 'self'))
#         assert resp.json['links']['first'] == f'/applications/{application.id}/datasets?page=1&per_page=20'
...
# #     def test_valid_get(self, client, application, valid_application2):
# #         resp = client.simulate_get(f"/applications/{application.id}/history")
# #         assert resp.status == HTTP_OK
# #         assert len(resp.json['data']) == 1
# #         assert resp.json
# #
# #         old_url = application.url
# #         application.url = new_url = "http://modified.app.com"
# #         application.save()
# #         resp = client.simulate_get(f"/applications/{application.id}/history")
# #         assert resp.status == HTTP_OK
...
# #         application.notes = new_notes = "Nowy opisik aplikacji"
# #         application.save()
# #
