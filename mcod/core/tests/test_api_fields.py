...
class TestLocalDateTimeField:
    ...
    def test_iso_format_in_local_timezone(
        self, local_tz, dt_input, dt_output, dt_utc_output
    ):
        """
        ...
        with patch(
            ...
            local_date = LocalDateTime.SERIALIZATION_FUNCS[
                self.DATETIME_FORMAT
            ](dt_input)
            utc_date = DateTime.SERIALIZATION_FUNCS[self.DATETIME_FORMAT](
                dt_input
            )
            mock_get_localzone.assert_called_once()
            assert (
                local_date == dt_output
            ), f"Expected {dt_output} but got {local_date}."
            assert (
                utc_date == dt_utc_output
            ), f"Expected {dt_utc_output} but got {utc_date}."
    ...
    def test_field_extends_serialization_funcs_for_iso8601t_format(self):
        ...
        assert (
            LocalDateTime.SERIALIZATION_FUNCS[self.DATETIME_FORMAT]
            == iso_format_in_local_timezone
