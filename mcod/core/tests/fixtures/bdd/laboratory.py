...
def dashboard_api_response_analyses(analyses, context):
    ...
    assert value == analyses, 'analyses on dashboard is %s, expected %s' % (value, analyses)


@given(parsers.parse('Laboratory analysis {labevent_id:d}'))
def analysis_with_id(labevent_id):
    analysis = LabEventFactory.create(id=labevent_id, title='Analysis with id: %s' % labevent_id, event_type='analysis')
...
def research_with_id(labevent_id):
    research = LabEventFactory.create(id=labevent_id, title='Research with id: %s' % labevent_id, event_type='research')
...
def labevent_with_id(labevent_id):
    lab_event = LabEventFactory.create(id=labevent_id, title='LabEvent with id: %s' % labevent_id,
