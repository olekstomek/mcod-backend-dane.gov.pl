...
class ByMonthItem(StatsBucketItem):
    title = ma.fields.String(date_key='date', data_key='key_as_string')


class DocumentsByTypeBucketItem(StatsBucketItem):
    title = ma.fields.String(data_key='type')
    by_month = ma.fields.Nested(ByMonthItem(), data_key='by_month.buckets', many=True)
    monthly_avg = ma.fields.Method('get_monthly_avg', data_key='load_monthly_avg')
...
class StatsAggregations(ArgsListToDict):
    documents_by_type = ma.fields.Nested(DocumentsByTypeBucketItem(), data_key='documents_by_type.buckets', many=True)
    datasets_by_institution = ma.fields.Nested(StatsBucketItem(app='organizations', model='Organization'),
                                               data_key='datasets_by_institution.inner.buckets', many=True)
    datasets_by_category = ma.fields.Nested(StatsBucketItem(app='categories', model='Category'),
                                            data_key='datasets_by_category.inner.buckets', many=True)
    datasets_by_formats = ma.fields.Nested(StatsBucketItem(), data_key='datasets_by_formats.buckets', many=True)
    datasets_by_tag = ma.fields.Nested(StatsBucketItem(), data_key='datasets_by_tag.buckets', many=True)
    datasets_by_openness_scores = ma.fields.Nested(StatsBucketItem(), data_key='datasets_by_openness_scores.buckets',
                                                   many=True)


class StatsMeta(SearchMeta):
    aggs = ma.fields.Nested(StatsAggregations, data_key='aggregations')
