...
class SparqlSignalProcessor:
    ...
    def update_graph(self, sender, instance, *args, **kwargs):
        update_graph_task.s(**self.get_task_kwargs(instance)).apply_async()

    def update_related_graph(self, sender, instance, *args, **kwargs):
        update_related_graph_task.s(**self.get_task_kwargs(instance)).apply_async()
    ...
    def update_graph_with_related(self, sender, instance, *args, **kwargs):
        update_graph_with_related_task.s(**self.get_task_kwargs(instance)).apply_async()

    def create_graph(self, sender, instance, *args, **kwargs):
        create_graph_task.s(**self.get_task_kwargs(instance)).apply_async()

    def create_graph_with_related_update(self, sender, instance, *args, **kwargs):
        create_graph_with_related_update_task.s(**self.get_task_kwargs(instance)).apply_async()
    ...
    def delete_graph(self, sender, instance, *args, **kwargs):
        ...
        if registry.is_parent_model(instance):
            delete_sub_graphs.s(**task_kwargs).apply_async()
        if not any(parents_to_remove):
            delete_graph_task.s(**task_kwargs).apply_async()
    ...
    def delete_graph_with_related_update(self, sender, instance, *args, **kwargs):
        ...
        if not any(parents_to_remove):
            ...
            delete_graph_with_related_update_task.s(**task_kwargs).apply_async()
