...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if data_type == 'all':
            ...
            if data_type not in methods_dict:
                raise CommandError('create_intermediate_files must have one of'
    ...
    def create_kibana_history_log(self, logs_path):
        ...
        sorted_main_file_data = OrderedDict([(date.strftime('%Y-%m-%d'), main_file_data[date.strftime('%Y-%m-%d')])
                                             for date in sorted_dates if date < last_date])
        log_data = []
    ...
    def aggregate_counter(self, downloads_counter, views_counter, pub_history_data):
        ...
        for dt, resources in dates_with_pub_resources.items():
            dt_date = dt.date()
            download_res_count = downloads_counter.filter(resource_id__in=resources, timestamp__lte=dt_date).aggregate(
                all_count=Sum('count'))['all_count'] or 0
            views_res_count = \
                views_counter.filter(
                    resource_id__in=resources, timestamp__lte=dt_date
                ).aggregate(all_count=Sum('count'))['all_count'] or 0
    ...
    def create_count_history(self):
        ...
        new_d = {}
        dt_d = {
            (datetime.strptime(date, '%Y-%m-%d').date() + relativedelta.relativedelta(days=1)).strftime('%Y-%m-%d'):
                date_data for date, date_data in d.items()}
