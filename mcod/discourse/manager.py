...
class DiscourseManager:
    ...
    def __init__(self, host=settings.DISCOURSE_HOST, sync_host=settings.DISCOURSE_SYNC_HOST,
                 api_key=settings.DISCOURSE_API_KEY, api_user=settings.DISCOURSE_API_USER):
        self.host = host
    ...
    def create_admin_api_key(self, **kwargs):
        ...
        if not (kwargs.get('password') and kwargs.get('username')):
            ...
            return
        client = DiscourseClientPasswordAuth(
            self.host, username=kwargs["username"],
            password=kwargs["password"])
        client.login()
        api_keys = client.list_api_keys()
        for key in api_keys['keys']:
            if key['user']['username'] == self.api_user and key['revoked_at'] is None:
                logger.debug(f'Api key for user {self.api_user} already exists, skipping creation'
                             f' and reading from env variable.')
                return
