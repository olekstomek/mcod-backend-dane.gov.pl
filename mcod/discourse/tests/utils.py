...
def discourse_response_mocker(user):
    ...
    mocked_urls['activate_mock_url'] =\
        settings.DISCOURSE_SYNC_HOST + '/admin/users/{0}/activate.json'.format(forum_user_id)
    ...
    mocked_urls['deactivate_mock_url'] =\
        settings.DISCOURSE_SYNC_HOST + '/admin/users/{0}/deactivate.json'.format(forum_user_id)
    ...
    with requests_mock.Mocker() as mock_request:
        mock_request.get(mocked_urls['external_id_mock_url'], json={'user': {'id': forum_user_id}},
                         headers={'Content-Type': 'application/json; charset=utf-8'})
        mock_request.post(mocked_urls['log_out_mock_url'], headers={'Content-Type': 'application/json; charset=utf-8'},
                          json={'success': 'ok'})
        mock_request.put(mocked_urls['deactivate_mock_url'],
                         headers={'Content-Type': 'application/json; charset=utf-8'}, json={'deactivated': True})
        mock_request.get(mocked_urls['mock_sso_url'])
        mock_request.post(mocked_urls['sync_sso_mock_url'], json={'username': username, 'id': forum_user_id},
                          headers={'Content-Type': 'application/json; charset=utf-8'})
        mock_request.post(mocked_urls['api_keys_mock_url'], json={'key': {'key': key}},
                          headers={'Content-Type': 'application/json; charset=utf-8'})
        mock_request.put(mocked_urls['activate_mock_url'],
                         headers={'Content-Type': 'application/json; charset=utf-8'}, json={'activated': True})
        mock_request.get(mocked_urls['list_api_keys_mock_url'],
                         headers={'Content-Type': 'application/json; charset=utf-8'},
                         json=[{'username': username, 'key': key, 'keyid': keyid}])
        mock_request.post(mocked_urls['revoke_key_mock_url'],
                          headers={'Content-Type': 'application/json; charset=utf-8'}, json={'revoked': True})
        mock_request.post(mocked_urls['undo_revoke_key_mock_url'],
                          headers={'Content-Type': 'application/json; charset=utf-8'}, json={'restored': True})
        mock_request.delete(mocked_urls['delete_api_key_mock_url'],
                            headers={'Content-Type': 'application/json; charset=utf-8'}, json={'deleted': True})
