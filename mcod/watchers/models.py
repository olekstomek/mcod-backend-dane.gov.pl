...
class ModelWatcherManager(models.Manager):
    ...
    def update_from_instance(self, instance, obj_state='updated', notify_subscribers=True, force=False, **kwargs):
        ...
        changed = force if force else watcher.tracker.has_changed('ref_value') or watcher.tracker.has_changed(
            'is_active')
        if changed:
...
class SearchQueryWatcherManager(models.Manager):
        ...
        for watcher in query:
            ...
            try:
                ...
                if new_value != old_value:
                    ...
                    SearchQueryWatcher.objects.update_from_url(watcher.object_ident, new_value, headers=headers,
                                                               obj_state=obj_state,
    ...
    def update_from_url(self, url, new_value, headers=None, obj_state='incresed', notify_subscribers=True, force=False):
        ...
        if changed:
            self.filter(id=watcher.id).update(
                last_ref_change=now(),
                ref_value=new_value
            )
...
def update_subscribed_object(sender, instance, *args, **kwargs):
    if instance.watcher.watcher_type == 'model':
        ...
        update_document_task.s(obj._meta.app_label, obj._meta.object_name, obj.id).apply_async(
            countdown=1)
