...
def decode_cipher_value(content):
    ...
    USER_ATTRS = tree.find(f".//{XMLENC}EncryptedData/{XMLENC}CipherData/{XMLENC}CipherValue").text
    ...
    ckdf = ConcatKDFHash(
        algorithm=hashes.SHA256(), length=32, otherinfo=binascii.unhexlify(otherinfo.encode()), backend=default_backend()
    )
