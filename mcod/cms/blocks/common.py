...
class HeaderBlock(blocks.StructBlock):
    size = HeaderChoiceBlock(
        label='Rozmiar',
        help_text='Rozmiar nagłówka.'
    )
...
class BannerBlock(blocks.StructBlock):
    image = ImageChooserBlock(required=True, label='Obrazek', help_text="""
                                Obrazek bannera. Zalecany rozmiar: 1920 x 340 pikseli.
                                """)
    action_url = blocks.URLBlock(label='Adres URL',
                                 help_text='Strona, na którą zostanie przekierowany '
                                           'użytkownik po naciśnięciu na baner.',
                                 required=True)
    target = LinkTargetChoiceBlock(label='Cel',
                                   help_text='Cel określa, czy strona w polu "Adres URL" '
                                             'ma się otwierać w tym samym, czy też w nowym oknie.',
                                   default='_self', required=False)
...
class CTAImageBlock(blocks.StructBlock):
    image = ImageChooserBlock(label='Obrazek', required=False,
                              help_text="Obrazek, który będzie wyświetlany obok "
                                        "tekstu. Zalecany rozmiar: 1920 x 340 pikseli.")
    position = AlignChoiceBlock(default='left', label='Położenie', required=False, help_text="""
                                Położenie obrazka względem tytułu oraz tekstu.
                                """)


class CTABackgroundBlock(blocks.StructBlock):
    image = ImageChooserBlock(label='Obraz w tle', required=False,
                              help_text="Obraz tła może być również animowanym gifem. "
                                        "Zalecany rozmiar: 1920 x 340 pikseli.")
    color = ColorPickerBlock(default='#ffffff', label='Kolor tła', required=False,
                             help_text="Kolor tła będzie zawsze pod spodem wszystkich "
                                       "innych elementów bloku (w tym również obrazu tła).")
...
class CTAButtonBlock(blocks.StructBlock):
    text = blocks.CharBlock(label='Tekst na przycisku', help_text="Maksymalnie 20 znaków.", max_length=20,
                            required=True)
    ...
    action_url = blocks.URLBlock(label='Adres URL',
                                 help_text='Strona, na którą zostanie przekierowany '
                                           'użytkownik po naciśnięciu przycisku.',
                                 required=True)
    target = LinkTargetChoiceBlock(label='Cel',
                                   help_text='Cel określa, czy strona w polu "Adres URL" '
                                             'ma się otwierać w tym samym, czy też w nowym oknie.',
...
class CTAHeaderBlock(blocks.StructBlock):
    ...
    align = AlignChoiceBlock(default='left', label='Wyrównanie', help_text="""
...
class CTATextBlock(blocks.StructBlock):
    text = blocks.TextBlock(max_length=200, rows=4, required=False, label='Tekst',
                            help_text="Treść tekstu wyświetlanego pod tytułem. Maksymalnie 4 linie, 200 znaków.")
    align = AlignChoiceBlock(default='left', required=False, label='Wyrównanie',
...
class CTABlock(blocks.StructBlock):
    ...
    button = CTAButtonBlock(label='Przycisk', )
