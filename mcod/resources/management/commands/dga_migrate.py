...
from mcod.resources.dga_constants import DGA_RESOURCE_EXTENSIONS, DGA_COLUMNS
from mcod.resources.models import Resource

MIGRATION_REPORT_COLUMNS: List[str] = ["Organizacja", "Rodzaj organizacji", "Nazwa zbioru danych", "Id zasobu",
                                       "Nazwa zasobu", "Status zasobu", "Uwagi"]

SEARCH_DGA_RESOURCE_NAME_OPTIONS: List[str] = ["chronionych danych", "danych chronionych"]

DGA_RESOURCE_TITLE_MUST_CONTAIN: List[str] = ["Wykaz zasobów chronionych", "wykaz zasobów chronionych"]
...
class DgaResourceValidationResult(Enum):
    ...
    OTHER_FLAG_ERROR = ("błąd ustawienia flagi `Zawiera dane dynamiczne` lub"
                        " `Zawiera dane o wysokiej wartości` lub `Zawiera dane badawcze`")
...
class DgaMigrator:
    ...
    def __init__(self, resource_name_filter: ResourceNameFilterBasedOnList, report_columns: List[str]):
    ...
    def get_public_organizations() -> QuerySet:
        qs = Organization.objects.filter(
            Q(institution_type=Organization.INSTITUTION_TYPE_LOCAL)
            | Q(institution_type=Organization.INSTITUTION_TYPE_STATE))
        return qs
    ...
    @staticmethod
    def get_organization_dga_datasources_for_organization(organization: Organization) -> QuerySet:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):  # noqa: C901
        ...
        resource_name_filter_based_on_list = ResourceNameFilterBasedOnList(
            name_options=SEARCH_DGA_RESOURCE_NAME_OPTIONS)
        dga_migrator = DgaMigrator(resource_name_filter=resource_name_filter_based_on_list,
                                   report_columns=MIGRATION_REPORT_COLUMNS)

        resource_validator = DgaResourceValidator(correct_extensions=DGA_RESOURCE_EXTENSIONS,
            ...
            if len(resources) == 1:
                ...
                if validation_result == DgaResourceValidationResult.CORRECT_VALIDATION:
                    if migrate_operation:  # Make migration
                        ...
                        self.stdout.write(f"Organization `{public_org.title}` - resource: {resource.title}"
                                          f" is qualified to DGA migration")

                    if not string_contains_element_from_list(resource.title, DGA_RESOURCE_TITLE_MUST_CONTAIN):
                        warning = (" - uwaga: tytuł zasobu nie zawiera żadnej z fraz: " +
                                   " | ".join(DGA_RESOURCE_TITLE_MUST_CONTAIN))
        ...
        dga_migrator.reports["public_institutions"].save_report(report_dir /
                                                                "RAPORT_Inst_PUBLICZNE_nieprawidłowości.csv")
        dga_migrator.reports["no_public_institutions"].save_report(report_dir /
                                                                   "RAPORT_Inst_NIEPUBLICZNE_nieprawidłowości.csv")
        dga_migrator.reports["migrations"].save_report(report_dir /
                                                       "RAPORT_migracja_realizacja.csv")
