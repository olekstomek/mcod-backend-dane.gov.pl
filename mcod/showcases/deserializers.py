...
class ShowcaseApiRequest(CommonSchema):
    id = search_fields.NumberField(
        _in='path', description='Showcase ID', example='447', required=True)
...
class CreateShowcaseProposalAttrs(ObjectAttrs):
    ...
    title = core_fields.Str(
        required=True, faker_type='showcase title', example='Some Showcase', validate=validate.Length(max=300))
