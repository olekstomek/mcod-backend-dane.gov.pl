...
class UserCSVSerializer(CSVSerializer):
    ...
    institution = fields.Str(attribute='institutions_ids_list_as_str', data_key=_('Institution'), example='1,2')
    institution1 = fields.Str(attribute='institutions_ids_list_as_str', data_key=_('Institution1'), example='1,2')
...
class UserSchemaMixin(object):
    state = fields.Str(
        ...
        description="Allowed values: 'active', 'inactive' or 'blocked'")
    email = fields.Email(
        required=True, faker_type='email', example='user@example.com')
    fullname = fields.Str(
        missing=None, faker_type='name', example='Jan Kowalski')
    about = fields.Str(
...
class UserApiAttrs(UserSchemaMixin, ObjectAttrs):
    is_newsletter_receiver = fields.Bool(
