Feature: Schedules list API
  Scenario Outline: Test schedule notifications list endpoint is not accessible for active user
  ...
  Scenario Outline: Test that some endpoints are accessible by admin and agent users only
  ...
  Scenario Outline: Test that specified endpoints returns related objects in included section
  ...
  Scenario Outline: Test that requests for export files returns link to tabular response
    Given logged <user_type>
    And schedule data created with {"schedule_id": 999, "user_schedule_id": 999, "user_schedule_item_id": 999}
    When api request method is GET
    And api request path is <request_path>
  ...
  Scenario: Test that opening of new schedule is available for admin
    Given logged admin user
  ...
  Scenario: Test that opening of new schedule is not available for agent user
  ...
  Scenario: Test that opening of new schedule returns error if currently planned schedule contains awaiting items
