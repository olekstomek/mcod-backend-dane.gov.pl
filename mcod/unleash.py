...
def is_enabled(feature_name: str, env_name: dict = settings.ENVIRONMENT, default_value: bool = False) -> bool:
    if not unleash_client or not unleash_client.is_initialized:
        logger.debug('UnleashClient was not initialized!')
        return False
    return unleash_client.is_enabled(feature_name, context={'envName': env_name}, default_value=default_value)
