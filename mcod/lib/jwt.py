...
def decode_jwt_token(auth_header):
    ...
    try:
        ...
        raise falcon.HTTPUnauthorized(title="401 Unauthorized", description=str(ex), code="token_error")
...
def get_auth_header(user, session_key, now=None, exp_delta=None):
    ...
    return "{header_prefix} {auth_token}".format(header_prefix=settings.JWT_HEADER_PREFIX, auth_token=auth_token)
