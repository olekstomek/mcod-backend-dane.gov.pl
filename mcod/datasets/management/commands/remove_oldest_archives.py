...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for directory in archive_dirs:
            non_symlinks = [os.path.join(directory, f) for f in os.listdir(directory)
                            if not os.path.islink(os.path.join(directory, f))]
            if len(non_symlinks) > 1:
                files_with_dt = sorted([(file_path, os.path.getmtime(file_path)) for file_path in non_symlinks],
                                       key=lambda x: x[1])
