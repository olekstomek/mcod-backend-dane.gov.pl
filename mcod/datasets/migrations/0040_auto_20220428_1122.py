...
def set_new_condition(apps, schema_editor):
    ...
    custom_descr_q = (~Q(license_condition_responsibilities="") &
                      Q(license_condition_responsibilities__isnull=False))
    objs = Dataset.objects.filter(Q(license_condition_source=True) |
                              Q(license_condition_modification=True) |
                              Q(license_condition_cc40_responsibilities=True) |
                              custom_descr_q).distinct()
...
def unset_new_conditions(apps, schema_editor):
    ...
    custom_descr_q = (~Q(license_condition_custom_description="") &
                      Q(license_condition_custom_description__isnull=False))
