...
def is_dcat_ap_conformant(rdf_content, ignore_warnings=False):
    ...
    for shape_path in settings.SHACL_SHAPES.values():
        conforms, results_graph, results_text = shacl_validate(
            graph, shacl_graph=shape_path, ont_graph=None,
            inference='rdfs',
            abort_on_error=False, meta_shacl=False, advanced=False, debug=False
        )
        if (ignore_warnings is False and conforms is False) or (None, SH.severity, SH.Violation) in results_graph:
            return False
    return True
...
def get_catalog_dcat_ap_triples(dataset, config):
    ...
    triples = [
        ...
        (catalog_ref, ns.DCT.description, Literal(config.CATALOG__DESCRIPTION_PL, lang='pl')),
        (catalog_ref, ns.DCT.description, Literal(config.CATALOG__DESCRIPTION_EN, lang='en')),
        (catalog_ref, ns.DCT.modified, Literal(resource.modified.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z',
                                               datatype=XSD.dateTime)),
        (catalog_ref, ns.DCT.issued, Literal(config.CATALOG__ISSUED, datatype=XSD.date)),
...
def get_catalog_schemaorg_triples(dataset, config):
    ...
    triples = [
        ...
        (catalog_ref, ns.SCHEMA.description, Literal(config.CATALOG__DESCRIPTION_PL, lang='pl')),
        (catalog_ref, ns.SCHEMA.description, Literal(config.CATALOG__DESCRIPTION_EN, lang='en')),
        (catalog_ref, ns.SCHEMA.dateModified, Literal(resource.modified.strftime('%Y-%m-%dT%H:%M:%S.%f')[:-3] + 'Z')),
...
def get_dataset_dcat_ap_triples(dataset, config):
    ...
    triples = [
        ...
        (dataset_ref, ns.DCT.title, Literal(dataset.title_pl or dataset.title, lang='pl')),
        (dataset_ref, ns.DCT.title, Literal(dataset.title_en or dataset.title, lang='en')),
        (dataset_ref, ns.DCT.description, Literal(dataset.notes_pl or dataset.notes, lang='pl')),
        (dataset_ref, ns.DCT.description, Literal(dataset.notes_en or dataset.notes, lang='en')),
        (dataset_ref, ns.DCT.issued, Literal(str(dataset.created).replace(' ', 'T'))),
        (dataset_ref, ns.DCT.modified, Literal(str(dataset.modified).replace(' ', 'T'))),
        ...
        *dataset_supplements_triples,
        ...
        (resource_ref, ns.DCT.title, Literal(resource.title_en or resource.title, lang='en')),
        (resource_ref, ns.DCT.description, Literal(resource.description_pl or resource.description, lang='pl')),
        (resource_ref, ns.DCT.description, Literal(resource.description_en or resource.description, lang='en')),
        *status_triples,
        (resource_ref, ns.DCT.issued, Literal(str(resource.created).replace(' ', 'T'), datatype=XSD.dateTime)),
        (resource_ref, ns.DCT.modified, Literal(str(resource.modified).replace(' ', 'T'), datatype=XSD.dateTime)),
        (resource_ref, ns.DCAT.accessURL, resource_ref),
        (resource_ref, ns.DCAT.downloadURL, URIRef(resource.download_url)),
        *format_triples,
        (resource_ref, ns.DCAT.byteSize, Literal(resource.file_size, datatype=XSD.decimal)),
        *media_type_triples,
        license_triple,
        *resource_supplements_triples,
...
def get_dataset_dcat_ap_pl_triples(dataset, config):
    ...
    if resource.openness_score:
        ...
        openness_score_triples.extend([
            ...
            (openness_score_ref, ns.SKOS.prefLabel, Literal(entry.name_pl, lang='pl')),
            (openness_score_ref, ns.SKOS.prefLabel, Literal(entry.name_en, lang='en')),
            (openness_score_vocab_ref, ns.RDF.type, ns.SKOS.ConceptScheme),
            (openness_score_vocab_ref, ns.DCT.title, Literal('Poziom otwartości', lang='pl')),
            (openness_score_vocab_ref, ns.DCT.title, Literal('Openness score', lang='en')),
        ])
    ...
    for sign in resource.special_signs.filter(status='published'):
        ...
        special_signs_triples.extend([
            ...
            *([(special_sign_ref, ns.SKOS.prefLabel, Literal(sign.name_en, lang='en'))] if sign.name_en else []),
            (special_sign_vocab_ref, ns.RDF.type, ns.SKOS.ConceptScheme),
            (special_sign_vocab_ref, ns.DCT.title, Literal('Znak umowny', lang='pl')),
            (special_sign_vocab_ref, ns.DCT.title, Literal('Special sign', lang='en')),
        ])
    ...
    if accrual_periodicity:
        ...
        accrual_periodicity_triples = [
            (dataset_ref, ns.DCT.accrualPeriodicity, accrual_periodicity_ref),
            (accrual_periodicity_ref, ns.RDF.type, ns.DCT.Frequency),
            (accrual_periodicity_ref, ns.SKOS.inScheme, URIRef(vocabs["frequency"].rstrip('/'))),
        ]
...
def get_dataset_schemaorg_triples(dataset):
    ...
    triples = [
        ...
        (dataset_ref, ns.SCHEMA.name, Literal(dataset.title_pl or dataset.title, lang='pl')),
        (dataset_ref, ns.SCHEMA.name, Literal(dataset.title_en or dataset.title, lang='en')),
        (dataset_ref, ns.SCHEMA.description, Literal(dataset.notes_pl or dataset.notes, lang='pl')),
        (dataset_ref, ns.SCHEMA.description, Literal(dataset.notes_en or dataset.notes, lang='en')),
        (dataset_ref, ns.SCHEMA.creativeWorkStatus, Literal(dataset.status)),
        (dataset_ref, ns.SCHEMA.dateCreated, Literal(str(dataset.created).replace(' ', 'T'))),
        (dataset_ref, ns.SCHEMA.dateModified, Literal(str(dataset.modified).replace(' ', 'T'))),
        ...
        *dataset_license_triples,
        ...
        (resource_ref, ns.SCHEMA.name, Literal(resource.title_en or resource.title, lang='en')),
        (resource_ref, ns.SCHEMA.description, Literal(resource.description_pl or resource.description, lang='pl')),
        (resource_ref, ns.SCHEMA.description, Literal(resource.description_en or resource.description, lang='en')),
        (resource_ref, ns.SCHEMA.creativeWorkStatus, Literal(resource.status)),
        (resource_ref, ns.SCHEMA.dateCreated, Literal(str(resource.created).replace(' ', 'T'))),
