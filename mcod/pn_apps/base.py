...
class StatsPanel(param.Parameterized):
    ...
    def __init__(self, user, lang, agent_type=None, doc=None, fav_charts_widget=None, theme_name='', **params):
        self.user = user
        ...
        self.fav_chart_1_widget = pn.widgets.Checkbox(
            name='1', width_policy='min', align='start', height_policy='min', css_classes=['fav-checkbox', ])
        self.fav_chart_2_widget = pn.widgets.Checkbox(
            name='2', width_policy='min', align='start', height_policy='min', css_classes=['fav-checkbox', ])
    ...
    def assign_adaptive_ticker(self, rendered_chart, desired_num_ticks=None):
        ...
        if self.agent_type and self.agent_type.is_mobile and plot_factors and not plot_factors[0].isnumeric():
            ...
            for interval in sub_ticker_intervals:
                sub_tickers.append(BasicTicker(
        ...
        rendered_chart.xaxis[0].formatter = FuncTickFormatter(
            code=conditional_formatter_code, args={'label_indexes': label_indexes, 'label_dict': label_overrides})
    ...
    def table(self):
        if self.show_table:
            ...
            row = pn.Row(pn.pane.HTML(
                self.table_template(df),
                sizing_mode='stretch_width', width_policy='max'),
                sizing_mode='stretch_width', width_policy='max', height_policy='max')
            end = time()
            total = end - start
...
class RawSqlOrganizationGroupStatsPanel(UserOrganizationGroupMixin, CombinedChartMixin, StatsPanel):
    ...
    def get_dataframe(self):
        ...
        options_visible_labels = \
            list(
                filter(lambda elem: elem[1] in selected_options, getattr(self.param, self.select_widget).names.items()))
        ...
        try:
            ...
            _data = OrderedDict({
                itm: {_('Entry'): i, 'values': dict(**data_dicts)} for i, itm in enumerate(periods)
            })
        ...
        df = self.reformat_dataframe(
            df, selected_options=selected_options,
            options_visible_labels=options_visible_labels, data_dicts=data_dicts)
        return df
