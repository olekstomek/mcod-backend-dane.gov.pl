...
class DatasetsCountByTimePeriod(StatsPanel):
    ...
    def get_queryset(self):
        ...
        return qs.annotate(
            period=Trunc('created', kind=self.timeperiod)
        ).values('period').annotate(**all_annotations).order_by('period').values(
            'dataset_count',
            'period',
            *list(single_type_counts.keys())
        )
    ...
    def get_dataframe(self):
        ...
        _data = OrderedDict({
            itm: {_('Entry'): i, 'values': dict(**viz_dicts)} for i, itm in enumerate(periods)
        })
...
class NewDatasetsCountByTimePeriod(StatsPanel):
    ...
    def get_queryset(self):
        ...
        all_annotations = {'dataset_count': overall_count_expr}
        all_annotations.update(single_type_counts)
        return qs.annotate(period=Trunc('created', kind=self.timeperiod)).values('period').annotate(
            **all_annotations
        ).order_by('period').values('period', 'dataset_count', *list(single_type_counts.keys()))
    ...
    def get_dataframe(self):
        ...
        if self.presentation_type == 'percentage':
            for v_key in viz_dicts.keys():
                ...
                df[v_key] = np.where(df[f'{v_key}_cumsum'] == 0, 0, (df[v_key] / df[f'{v_key}_cumsum']) * 100)
    ...
    def get_dataframe(self):
        ...
        if self.presentation_type == 'percentage':
            for rt in _res_types:
                ...
                df[data_label] =\
                    np.where(df[f'{data_label}_cumsum'] == 0, 0, (df[data_label] / df[f'{data_label}_cumsum']) * 100)
...
class TopResourceCountOrgfromGroup(UserFilterMixin, RankingPanel):
    ...
    def get_dataframe(self):
        ...
        org_names = Organization.objects.filter(
            user_q
        ).exclude(pk__in=org_ids).values_list('title', flat=True).order_by('-pk')
...
class Top10LastMonthOrganizationNewDatasets(RankingPanel):
    ...
    def get_queryset(self):
        ...
        qs = qs.annotate(**selected_types)
        subquery =\
            subquery.annotate(**selected_types).filter(
                organization_id=OuterRef('organization_id')).order_by().values('organization_id')
        count_filter = None
        single_type_counts = {}
        ...
        for selected_type in selected_types.keys():
            ...
            if self.presentation_type == 'percentage':
                ...
                single_type_counts[f'previous_{type_name}_count'] =\
                    Coalesce(Subquery(
                        subquery.annotate(**sub_annotate).values(sub_annotate_label), output_field=IntegerField()), 0)
                when_condition = {f'previous_{type_name}_count': 0,
                                  'then': 0}
                percentage_annotations[f'{type_name}_percentage'] =\
                    Case(When(**when_condition),
                         default=Cast(F(f'{type_name}_count'), FloatField()) /
                         Cast(F(f'previous_{type_name}_count'), FloatField()))
        overall_count_expr = Count('pk', filter=count_filter, distinct=True) if count_filter else Count('pk')
        all_annotations = {'dataset_count': overall_count_expr}
        all_annotations.update(single_type_counts)
        ...
        if self.presentation_type == 'percentage':
            all_annotations.update(
                {'previous_dataset_count': Coalesce(
                    Subquery(subquery.annotate(sub_datasets_count=overall_count_expr).values('sub_datasets_count'),
                             output_field=IntegerField()), 0)})
            values_list.append('previous_dataset_count')
        qs = qs.values('organization_id').annotate(**all_annotations)
        if self.presentation_type == 'percentage':
            percentage_annotations['dataset_percentage'] =\
                Case(When(previous_dataset_count=0, then=0),
...
class CountChartByTimePeriod(ChartPanel):
    ...
    def get_queryset(self):
        ...
        qs = qs.annotate(
            ...
            'period', 'count_sum').order_by('period')
        return qs
...
class OrganizationGroupCountByTimePeriod(UserFilterMixin, CombinedChartMixin, CountChartByTimePeriod):
    ...
    def get_queryset(self):
        ...
        if selected_organizations:
            ...
            group_values.extend(['resource__dataset__organization__title', 'resource__dataset__organization__pk'])
        else:
        ...
        qs = qs.annotate(
            ...
            'count_sum', *group_values).order_by('period')
        return qs
    ...
    def get_dataframe(self):
        if self.organizations:
            ...
            column_name_mapping = \
                {org['pk']: org['title'] for
                 org in mapping_details}
            try:
                periods = prepare_time_periods(qs[0]['period'], qs[-1]['period'], self.timeperiod)
                _data = OrderedDict()
                for i, itm in enumerate(periods):
...
class OrganizationGroupDatasetObservationByTimePeriod(UserOrganizationGroupMixin, CombinedChartMixin, ChartPanel):
    ...
    def get_queryset(self):
        ...
        ds = Dataset.objects.filter(dataset_q, pk=Cast(OuterRef('watcher__object_ident'), IntegerField()),
                                    ).values('organization_id')
        qs = Subscription.objects.filter(watcher__object_name='datasets.Dataset').annotate(
            period=Trunc('created', kind=self.timeperiod)).values('period').annotate(
            sub_org_id=Subquery(ds, output_field=CharField())
        ).filter(sub_org_id__isnull=False).values(*grouping_vals)
        if self.organizations:
    ...
    def get_org_specific_data(self, data, qs):
        ...
        column_name_mapping = \
            {org['pk']: org['title'] for org in mapping_details}
        try:
            periods = prepare_time_periods(qs[0]['period'], qs[-1]['period'], self.timeperiod)
            _data = OrderedDict()
            for i, itm in enumerate(periods):
...
class Top10ObservedDatasets(RankingPanel):
    ...
    def get_queryset(self):
        ...
        qs = Subscription.objects.filter(
            ...
            dataset_title=Subquery(ds, output_field=CharField())
        ).values('dataset_title', 'watcher__object_ident').annotate(
            subs_count=Count('watcher__object_ident')).values('dataset_title', 'subs_count')
...
class DatasetsCountByTimePeriodForGroup(DataCountByTimePeriodForGroup):
    ...
    default_subquery = "SUM(COUNT(d.id)) OVER(PARTITION BY o.title ORDER BY " \
...
class ResourcesForInstitutionsGroupByTimePeriod(DataCountByTimePeriodForGroup):
    ...
    default_subquery = "SUM(COUNT(r.id)) OVER(PARTITION BY o.title ORDER BY " \
...
def app(doc):
    ...
    if user.is_authenticated:
        ...
        charts = [
            Top10ProvidersByResourcesCount(name='Ranking instytucji o największej liczbie danych',
                                           **charts_kwargs),
            ...
            Top10ProvidersByDatasetsCount(
                name='Ranking instytucji o największej liczbie zbiorów danych', **charts_kwargs),
            DatasetsCountByTimePeriod(name=_('Number of datasets'), **charts_kwargs),
            NewDatasetsCountByTimePeriod(
                name=_('Number of datasets added'), **charts_kwargs),
            DatasetsCountByTimePeriodForGroup(
                name='Liczba zbiorów danych dla grupy instytucji', **charts_kwargs),
            NewDatasetsByTimePeriodForGroup(
                name='Liczba nowych zbiorów danych dla grupy instytucji', **charts_kwargs),
            NewResourcesForInstitutionsGroupByTimePeriod(
                name='Liczba nowych danych dla grupy instytucji', **charts_kwargs),
            ResourcesForInstitutionsGroupByTimePeriod(name='Liczba danych dla grupy instytucji', **charts_kwargs),
            Top10MostDownloadedDatasets(
                name=_('Most downloaded datasets'), **charts_kwargs
            ),
            Top10MostViewedDatasets(
                name=_('Most viewed datasets'), **charts_kwargs
            ),
            Top10OrganizationsMostDownloadedDatasets(
                name='Ranking instytucji o największej liczbie pobrań', **charts_kwargs),
