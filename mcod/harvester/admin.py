...
class DataSourceImports(PaginationInline):
    ...
    def get_fields(self, request, obj=None):
        return ('_start', 'end', '_status', '_error_desc', )
...
class DataSourceAdmin(HistoryMixin, ModelAdmin):
    ...
    add_addanother_msg_template = _(
        'The data source "{obj}" was added successfully. You may add another data source below.')
    ...
    change_addanother_msg_template = _(
        'The data source "{obj}" was changed successfully. You may add another {name} below.')
    ...
    def get_urls(self):
        urls = super().get_urls()
        extra_urls = [
            re_path(r'^validate_xml/(?P<task_id>[\w-]+)/$', staff_member_required(get_progress),
    ...
    def save_model(self, request, obj, form, change):
        ...
        if is_import_needed:
            import_data_task.s(obj.id, force=True).apply_async_on_commit(countdown=2)
