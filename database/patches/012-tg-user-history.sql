...
/* FUNKCJA POD TRIGGER */
CREATE OR REPLACE FUNCTION public.fn_insert_user_history()
  RETURNS TRIGGER
LANGUAGE plpgsql
AS $function$
...
BEGIN
  ...
  v_functionname := 'fn_insert_user_history';
...
BEGIN;
/* TRIGGER NA TABELI */
CREATE TRIGGER tg_user_history
  AFTER INSERT OR UPDATE OR DELETE
  ON public.user
  FOR EACH ROW EXECUTE PROCEDURE public.fn_insert_user_history();
