...
/* FUNKCJA POD TRIGGER */
CREATE OR REPLACE FUNCTION public.fn_insert_tag_history()
  RETURNS TRIGGER
LANGUAGE plpgsql
AS $function$
...
BEGIN
  ...
  v_functionname := 'fn_insert_tag_history';
...
BEGIN;
/* TRIGGER NA TABELI */
CREATE TRIGGER tg_tag_history
  AFTER INSERT OR UPDATE OR DELETE
  ON public.tag
  FOR EACH ROW EXECUTE PROCEDURE public.fn_insert_tag_history();
