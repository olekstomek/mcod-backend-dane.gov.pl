...
/* FUNKCJA POD TRIGGER */
CREATE OR REPLACE FUNCTION public.fn_insert_article_history()
  RETURNS TRIGGER
LANGUAGE plpgsql
AS $function$
...
BEGIN
  ...
  v_functionname := 'fn_insert_article_history';
...
BEGIN;
/* TRIGGER NA TABELI */
CREATE TRIGGER tg_article_history
  AFTER INSERT OR UPDATE OR DELETE
  ON public.article
  FOR EACH ROW EXECUTE PROCEDURE public.fn_insert_article_history();
