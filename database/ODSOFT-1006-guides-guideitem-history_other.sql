...
/* FUNKCJA POD TRIGGER */
CREATE OR REPLACE FUNCTION public.fn_insert_guides_guideitem_history()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $function$
...
BEGIN
    ...
    v_functionname := 'fn_insert_guides_guideitem_history';
    ...
    SELECT
        ...
        NEW.guide_id,
        TG_OP,
        json_build_object('guide_item', row_to_json(NEW)),
    ...
    SELECT
        ...
        OLD.guide_id,
        TG_OP,
        json_build_object('guide_item', row_to_json(OLD)),
        json_build_object('guide_item', row_to_json(NEW)),
        CAST(current_setting('myapp.userid') AS INTEGER);
...
/* TRIGGER NA TABELI */
CREATE TRIGGER tg_guides_guideitem_history
AFTER INSERT OR UPDATE OR DELETE ON public.guides_guideitem FOR EACH ROW EXECUTE PROCEDURE public.fn_insert_guides_guideitem_history();
