...
/* FUNKCJA POD TRIGGER */
CREATE OR REPLACE FUNCTION public.fn_insert_user_agent_organizations_history()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $function$
...
BEGIN
    ...
    v_functionname := 'fn_insert_user_agent_organizations_history';
    ...
    SELECT
        ...
        NEW.user_id,
        TG_OP,
        json_build_object('field', 'agent_organizations', 'organization_id', NEW.organization_id),
    ...
    SELECT
        ...
        OLD.user_id,
        TG_OP,
        json_build_object('field', 'agent_organizations', 'organization_id', OLD.organization_id),
        json_build_object('field', 'agent_organizations', 'organization_id', NEW.organization_id),
        CAST(current_setting('myapp.userid') AS INTEGER);
    ...
    /* logowanie prawidłowego wywołania procedury */
...
/* TRIGGER NA TABELI */
CREATE TRIGGER tg_user_agent_organizations_history
AFTER INSERT OR UPDATE OR DELETE ON public.user_agent_organizations FOR EACH ROW EXECUTE PROCEDURE public.fn_insert_user_agent_organizations_history();
