...
/* FUNKCJA POD TRIGGER */
CREATE OR REPLACE FUNCTION public.fn_insert_newsletter_history()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $function$
...
BEGIN
    ...
    v_functionname := 'fn_insert_newsletter_history';
...
/* TRIGGER NA TABELI */
CREATE TRIGGER tg_newsletter_history
AFTER INSERT OR UPDATE OR DELETE ON public.newsletter FOR EACH ROW EXECUTE PROCEDURE public.fn_insert_newsletter_history();
...
/* FUNKCJA POD TRIGGER */
CREATE OR REPLACE FUNCTION public.fn_insert_newsletter_submission_history()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $function$
...
BEGIN
    ...
    v_startdatetime := clock_timestamp();
    v_functionschema := 'public';
    v_functionname := 'fn_insert_newsletter_submission_history';
...
/* TRIGGER NA TABELI */
CREATE TRIGGER tg_newsletter_submission_history
AFTER INSERT OR UPDATE OR DELETE ON public.newsletter_submission FOR EACH ROW EXECUTE PROCEDURE public.fn_insert_newsletter_submission_history();
...
/* FUNKCJA POD TRIGGER */
CREATE OR REPLACE FUNCTION public.fn_insert_newsletter_subscription_history()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $function$
...
BEGIN
    ...
    v_startdatetime := clock_timestamp();
    v_functionschema := 'public';
    v_functionname := 'fn_insert_newsletter_subscription_history';
...
/* TRIGGER NA TABELI */
CREATE TRIGGER tg_newsletter_subscription_history
AFTER INSERT OR UPDATE OR DELETE ON public.newsletter_subscription FOR EACH ROW EXECUTE PROCEDURE public.fn_insert_newsletter_subscription_history();
