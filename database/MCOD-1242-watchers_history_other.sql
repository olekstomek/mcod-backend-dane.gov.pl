...
/* FUNKCJA POD TRIGGER */
CREATE OR REPLACE FUNCTION public.fn_insert_watchers_notification_history()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $function$
...
Description:				procedura pod trigger - insert do public.History_other
Last modify by:				
Last modified date:			
...
BEGIN
    ...
    v_functionname := 'fn_insert_watchers_notification_history';


    /* wstawienie danych do historii */
    IF TG_OP = 'INSERT' THEN
        
        INSERT INTO public.History_other
        (
...
/* TRIGGER NA TABELI */
CREATE TRIGGER tg_watchers_notification_history
AFTER INSERT OR UPDATE OR DELETE ON public.watchers_notification FOR EACH ROW EXECUTE PROCEDURE public.fn_insert_watchers_notification_history();



/* 2. TG - watchers_subscription */
...
/* FUNKCJA POD TRIGGER */
CREATE OR REPLACE FUNCTION public.fn_insert_watchers_subscription_history()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $function$
...
BEGIN
    ...
    v_startdatetime :=  clock_timestamp();
    v_functionschema := 'public';
    v_functionname := 'fn_insert_watchers_subscription_history';
...
/* TRIGGER NA TABELI */
CREATE TRIGGER tg_watchers_subscription_history
AFTER INSERT OR UPDATE OR DELETE ON public.watchers_subscription FOR EACH ROW EXECUTE PROCEDURE public.fn_insert_watchers_subscription_history();



/* 3. TG - watchers_watcher */
...
/* FUNKCJA POD TRIGGER */
CREATE OR REPLACE FUNCTION public.fn_insert_watchers_watcher_history()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $function$
...
BEGIN
    ...
    v_startdatetime :=  clock_timestamp();
    v_functionschema := 'public';
    v_functionname := 'fn_insert_watchers_watcher_history';
...
/* TRIGGER NA TABELI */
CREATE TRIGGER tg_watchers_watcher_history
AFTER INSERT OR UPDATE OR DELETE ON public.watchers_watcher FOR EACH ROW EXECUTE PROCEDURE public.fn_insert_watchers_watcher_history();
